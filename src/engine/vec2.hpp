...
template <typename T = float> struct Vec2 {
    ...
    template <typename U> constexpr Vec2(const Vec2<U>& o) : x(static_cast<T>(o.x)), y(static_cast<T>(o.y)) {}
    ...
    template <typename S> Vec2<S> as() const { return {static_cast<S>(x), static_cast<S>(y)}; }

    Vec2<float> round() const
...
template <typename T> const Vec2<T> Vec2<T>::Origin{0, 0};
